...
2. **Use Tensorboard**. Used a run builder based on [this post](https://hackernoon.com/how-to-structure-a-pytorch-ml-project-with-google-colab-and-tensorboard-7ram3agi) but uses tensorboard magic in colab instead of ngrok method. [My Runbuilder code link](https://github.com/abhinavdayal/EVA4_LIBRARY/blob/master/EVA4/runbuilder.py)
...
5. **LR Scheduler**: Used One Cycle Policy for initial training then cyclic LR with triangular 2 policy. User my [LR Range finder](https://github.com/abhinavdayal/EVA4_LIBRARY/blob/master/EVA4/eva4LRFinder2.py).
...
8. As an input I **only passed fgbg image and not the background image**. WHY? I felt that passing bg will make task easier. However post facto I am realizing that it may be better to help netwiork learn early initially on a portion of data and then train again on another portion without bg image explicitly provided. As an output I planned to output 2 channels, one for the mask and other for the depth.
9. Finally I tried the network on sample 8 images that are real images and not generated images to see how the network is performing on data that is not seen by the network at all. The results were awesone.
...
3. Pixel shuffle is good but that does not mean transpose convs need not be used anywhere. In fact in the [final model]((https://github.com/abhinavdayal/EVA4_LIBRARY/blob/master/EVA4/eva4models/s15netED.py), I observe that I am doing pixel shuffle too early. After concatenating the previous layer and skip connection input, I should first do some convs and then do pixel shuffle, something like [my this draft code](https://github.com/abhinavdayal/EVA4_LIBRARY/blob/master/EVA4/eva4models/s15netED2.py). However I thought of that later so could not try.
...
8. Since I ran the training 2-3 times, every time my data loader is shuffling to create train-test split ( as I have to run it again because of colab restrictions, there is a possibility of some leakage as I forgot to use the seed, but still on unseen data it is doing well.
